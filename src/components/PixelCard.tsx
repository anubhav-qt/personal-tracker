...
// Define type for the Pixel class
class Pixel {
  width: number;
  height: number;
...
export default function PixelCard({
  ...
  // Safe initial values that won't cause errors during SSR
  const timePreviousRef = useRef<number>(0);
  const reducedMotionRef = useRef<boolean>(false);

  // Set up browser-specific values after mount
  useEffect(() => {
    setIsBrowser(true);
    timePreviousRef.current = window.performance.now();
  ...
  const initPixels = () => {
    if (!containerRef.current || !canvasRef.current || !isBrowser) return;
  ...
  useEffect(() => {
    ...
    return undefined;
  }, [isBrowser, finalGap, finalSpeed, finalColors, finalNoFocus]);
